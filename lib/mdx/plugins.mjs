...
/*
 * Read dimensions from a WebP buffer (VP8 / VP8L / VP8X).
 *
 * The container is RIFF: a 12-byte header (RIFF / size / WEBP) followed by a
 * chunk whose FourCC tells us the encoding. Chunk data starts at byte 20; we
 * parse the fixed-width fields from there, matching image-size's decoder.
 *
 * Note the two different conventions: VP8X and VP8L store `size - 1` and need
 * the `+ 1` correction, but the VP8 lossy key-frame header stores the real
 * dimensions directly (RFC 6386 §9.1), so it must not be incremented.
 *
 * @param {Buffer} buffer
 * @returns {{ width: number, height: number } | null}
 */
function readWebpDimensions(buffer) {
  ...
  if (chunkHeader === 'VP8X') {
    ...
    width = ((b(7) << 8) | b(6)) & 0x3fff
    height = ((b(9) << 8) | b(8)) & 0x3fff
  } else if (chunkHeader === 'VP8L') {
    const bits = (b(4) << 24) | (b(3) << 16) | (b(2) << 8) | b(1)
    width = (bits & 0x3fff) + 1
 ...
 *
 * The local implementation supports PNG, JPEG, SVG, and WebP — the formats
 * used by the template assets. Add an AVIF parser here if the blog starts
 * authoring that format in MDX.
 *
